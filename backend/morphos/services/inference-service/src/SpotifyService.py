...
class SpotifyService:
    ...
    def __init__(self):
        ...
        if not self.client_id or not self.client_secret:
            print(
                "WARNING: Spotify credentials not set. Please set SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET environment variables."
            )
            self.sp = None
        else:
        ...
        try:
            ...
            try:
                ...
                artist_results = self.sp.search(
                    q="artist:Drake", type="artist", limit=1
                )
                if (
                    artist_results
                    and "artists" in artist_results
                    and artist_results["artists"]["items"]
                ):
                    artist_id = artist_results["artists"]["items"][0]["id"]

                    # Get recommendations using the artist
                    results = self.sp.recommendations(
                        seed_artists=[artist_id], limit=limit
                    )
